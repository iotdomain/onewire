...
func (app *OnewireApp) updateSensor(nodeID string, sensorNode *XMLNode) {
	...
	output := pub.GetOutput(nodeID, sensorType, types.DefaultOutputInstance)
	if output == nil {
		// convert OneWire EDS data type to IoTDomain output types
		rawUnit := sensorNode.Units
		output = pub.NewOutput(nodeID, sensorType, types.DefaultOutputInstance)
		...
		if strings.ToLower(sensorNode.Writable) == "true" {
			pub.NewInput(nodeID, types.InputType(sensorType), types.DefaultInputInstance)
		}
	}

	newVal := string(sensorNode.Content)
	pub.UpdateOutputValue(nodeID, sensorType, types.DefaultOutputInstance, newVal)
}
...
func (app *OnewireApp) updateDevice(deviceOWNode *XMLNode) {
	...
	pub.UpdateNodeAttr(nodeID, map[types.NodeAttr]string{
